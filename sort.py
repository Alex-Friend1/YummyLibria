...
class MainWindow(QMainWindow):
    def __init__(self, username):
        ...
        self.sort_options = ['По умолчанию', 'По названию (А-Я)', 'По названию (Я-А)',
                             'По времени (быстрее)', 'По времени (дольше)']
        self.initUI()
    ...
    def initUI(self):
        ...
        # Заголовок
        title_label = QLabel('Выберите десерт для просмотра рецепта')
        title_font = QFont()
        ...
        search_label = QLabel("🔍 Поиск десертов:")
        search_label.setStyleSheet("font-weight: bold;")
        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("Введите название десерта или ингредиент...")
        ...
        # Фильтр по народам
        filter_layout.addWidget(QLabel("🌍 Народ:"))
        ...
        #     }
        # """)
        # self.sort_combo.currentTextChanged.connect(self.sort_desserts)
        ...
        filter_layout.addWidget(stats_button)
        ...
        self.count_label = QLabel(f"Всего десертов: {len(self.all_desserts)}")
    ...
    def create_dessert_widgets(self):
        ...
        # Обновляем счетчик
        self.count_label.setText(f"Показано: {len(self.filtered_desserts)} из {len(self.all_desserts)} десертов")
        ...
        if not self.filtered_desserts:
            ...
            no_results_text = QLabel("Десерты не найдены")
    ...
    #     self.sort_desserts()
    #
    # def sort_desserts(self):
    #     # Сортировка десертов по выбранному критерию
    #     sort_option = self.sort_combo.currentText()
    #
    #     if sort_option == 'По умолчанию':
    #         # Исходный порядок (как в файле)
    #         pass
    #     elif sort_option == 'По названию (А-Я)':
    #         self.filtered_desserts.sort(key=lambda x: x['name'])
    #     elif sort_option == 'По названию (Я-А)':
    #         self.filtered_desserts.sort(key=lambda x: x['name'], reverse=True)
    #     elif sort_option == 'По времени (быстрее)':
    #         # Парсим время приготовления
    #         def parse_time(time_str):
    #             total_minutes = 0
    #             if 'час' in time_str:
    #                 hours_part = time_str.split('час')[0].strip()
    #                 if hours_part:
    #                     total_minutes += int(hours_part) * 60
    #             if 'минут' in time_str:
    #                 minutes_part = time_str.split('минут')[0].split()[-1]
    #                 if minutes_part.isdigit():
    #                     total_minutes += int(minutes_part)
    #             return total_minutes
    #
    #         self.filtered_desserts.sort(key=lambda x: parse_time(x['time']))
    #     elif sort_option == 'По времени (дольше)':
    #         def parse_time(time_str):
    #             total_minutes = 0
    #             if 'час' in time_str:
    #                 hours_part = time_str.split('час')[0].strip()
    ...
    #             return total_minutes
    #
    #         self.filtered_desserts.sort(key=lambda x: parse_time(x['time']), reverse=True)
    ...
    def reset_filters(self):
        ...
        QMessageBox.information(self, "Фильтры сброшены",
                                "Все фильтры были сброшены. Показаны все десерты.")
    ...
    def show_statistics(self):
        ...
        for category, count in sorted(category_stats.items()):
            message += f"• {category}: {count} десерт(ов)\n"

        message += "\nПо народам:\n"
        for nation, count in sorted(nation_stats.items()):
            message += f"• {nation}: {count} десерт(ов)\n"

        message += f"\nВсего десертов: {len(self.all_desserts)}"
